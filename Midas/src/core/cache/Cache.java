...
public class Cache {
   ...
   /*
    * Usage d'une HashMap pour plusieurs raisons : - Une fois la référence
    * insérée il faudrait beaucoup d'objets en mémoire pour qu'elle soit libérée
    * par le ramasse-miettes. Ou encore vraiment peu de mémoire sur sa machine.
    * La HashMap a un coût d'insertion, mais un coût faible en lecture qui sera
    * sa principale activité. - Les identifiants étant uniques, il n'y aura pas
    * de collision pour une même valeur, supprimant ainsi un problème des
    * HashMap.
    * 
   ...
   private HashMap<Class<? extends IdentifiedComponent>,
                   HashMap<Integer,
                           SoftReference<? extends IdentifiedComponent>>>
            cache = new HashMap<>();

   /**
    * Crée un cache pour les objets du coeur.
    */
   public Cache() {
    ...
    * 
    * @param component
    *           - le composant à conserver dans le cache.
    */
   public <T extends IdentifiedComponent> void putToCache(T component) {
      ...
      SoftReference<? extends IdentifiedComponent> reference = map
            .get(component.getId());
   ...
   /**
    * Retourne les composants identifiables de la classe donnée présents dans le
    * cache.
    * 
    * @param type
   ...
   /**
    * Retourne l'objet à partir d'une référence donnée.
    * <p>
    * <b><u>Attention :</u></b> ne vérifie pas le type. Cette méthode est à
    * usage purement interne afin de masquer le warning de conversion sans
    * utiliser de @SuppressWarnings sur la méthode publique.
    * 
    * @param softRef
    *           - la référence.
    ...
    */
   @SuppressWarnings("unchecked")
   private <T extends IdentifiedComponent> T getRef(
         SoftReference<? extends IdentifiedComponent> softRef) {
      return (T) softRef.get();
   }
